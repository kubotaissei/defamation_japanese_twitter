...
class BiRNN(nn.Module):
    def __init__(self, args, embeddings):
        ...
        self.embedding.weight = nn.Parameter(
            torch.tensor(embeddings.astype(np.float32), dtype=torch.float32)
        )
    ...
    def forward(
        ...
        h_embedding = torch.squeeze(
            self.dropout_embed(torch.unsqueeze(h_embedding, 0))
        ).view(batch_size, input_ids.shape[1], self.embedsize)
        ...
        if labels is not None:
            loss_funct = torch.nn.CrossEntropyLoss(
                weight=torch.tensor(self.weights).to(device), reduction="mean"
            )
...
class LSTM_bad(BiRNN):
    ...
    def forward(self, x, x_mask):
        ...
        h_embedding = torch.squeeze(
            self.dropout_embed(torch.unsqueeze(h_embedding, 0))
        ).view(batch_size, x.shape[1], self.embedsize)
        _, hidden = self.seq_model(h_embedding)
        hidden = hidden[0]
        if debug:
            print(hidden.shape)
...
class CNN_GRU(BiRNN):
    def __init__(self, args, embeddings):
        ...
        self.seq_model = nn.GRU(
            100, 100, bidirectional=False, batch_first=True, dropout=self.drop_hidden
        )
...
class BiAtt_RNN(BiRNN):
    def __init__(self, args, embeddings, return_att):
        ...
        if args["attention"] == "sigmoid":
            self.seq_attention = Attention_LBSA_sigmoid(
                self.hidden_size * 2, args["max_length"]
            )
        else:
            self.seq_attention = Attention_LBSA(
                self.hidden_size * 2, args["max_length"]
            )
    ...
    def forward(
        ...
        h_embedding = torch.squeeze(
            self.dropout_embed(torch.unsqueeze(h_embedding, 0))
        ).view(input_ids.shape[0], input_ids.shape[1], self.embedsize)
        ...
        if labels is not None:
            ...
            loss_logits = loss_funct(
                outputs[0].view(-1, self.num_labels), labels.view(-1)
            )
            loss = loss_logits

            if self.train_att:
                loss_atts = self.lam * masked_cross_entropy(
                    att, attention_vals, attention_mask
                )
                loss = loss + loss_atts
